...
# MuMIn::r.squaredGLMM(biomass_model)
...
scaled_diam_model <- lmer(log(avg_diameter) ~ scale(distance)*scale(pct_moisture) + scale(avg_pct_light) + age + (1|site_code) + (1|common_name), data = site_grouped_data)
summary(scaled_diam_model)

```

```{r plot diameter model}
...
# str(diam_fixed)
ggplot(filter(diam_fixed, var_names!="Intercept"), 
       aes(y = var_names, x = Estimate)) +
  # coord_flip() +
  geom_vline(xintercept = 0, color = "gray70") +
  geom_point(size = 2) +
...
MuMIn::r.squaredGLMM(diam_model)
...
form <- formula(avg_rgr_ht ~ avg_pct_light + age + distance * pct_moisture + (1 | site_code) + (1 | common_name))
rgr_model <- lmer(form, data = site_grouped_data, REML = F)

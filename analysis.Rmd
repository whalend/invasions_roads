...
seedling_data <- seedling_data %>%
  ...
  left_join(., grouped_forest_data, by = c("age","site_code","distance")) %>% 
  left_join(., grouped_light_data, by = c("age","site_code","distance")) %>% 
  left_join(., litter_data, by = c("age","site_code","distance")) %>% 
  left_join(., soil_moisture_data, by = c("age","site_code","distance"))
...
# names(seedling_data)
seedling_data <- mutate(seedling_data, common_name = "")
...
seedling_data$forest_age[seedling_data$forest_age=="young"] <- "Young forest"
seedling_data$forest_age[seedling_data$forest_age=="mature"] <- "Mature forest"
...
light_data
ggplot(seedling_data, aes(pct_light, final_ht06)) +
...
This calculation of relative growth is just the proportional difference between the initial height and final height (doesn't account for time, so no rate). There are (`r n_distinct(filter(seedling_data, rgr_ht<0))`) negative values (range (%) = `r range(100*filter(seedling_data, rgr_ht<0)$rgr_ht)`). 
...
surv <- surv %>% 
  group_by(year, distance, forest_age, status, common_name) %>% 
  filter(!is.na(survival)) %>% 
  summarise(pct_survival = 100*sum(survival)/length(survival))

ggplot(surv, aes(distance, pct_survival)) +
  stat_summary(aes(shape = common_name), fun.data = "mean_se", geom = "point", size = 2.5) +
  geom_smooth(aes(linetype = status), method = "lm", se = F, show.legend = F, color = "grey50") +
  scale_linetype_manual(values = c(2,1)) +
  scale_color_brewer(palette = "RdBu") +
...
Average proportional survival in each year.
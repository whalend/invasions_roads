...
diam_sjplot_rislope <- sjPlot::sjp.lmer(diam_model, type = "ri.slope", prnt.plot = F, transform="exp")
...
(rgr_species_ranef_plot <- ggplot(filter(rgr_ranef_df, grpvar=="common_name",term!="(Intercept)"), 
       aes(y = grp, x = condval)) +
  geom_point() +
  geom_errorbarh(aes(xmin = condval - 2*condsd, xmax = condval + 2*condsd), height = 0) +
  geom_vline(xintercept = 0, color = "gray70") +
...
```{r growth-fixed-coefs-fig1, fig.width=8, fig.height=3}
...
), nrow = 1, rel_widths = c(1.6,1,1), scale = .93, labels = "auto", label_x = c(.4,.1,.1))

# ggsave("figures/growth-scaled-coefs-fig.eps", width = 8, height = 3)

```

```{r species-growth-diff-fig2, fig.width=9, fig.height=10}
...
cowplot::plot_grid(plotlist = list(
  biomass_species_ranef_plot, biomass_distance_lines,  
  diam_species_ranef_plot, diam_distance_lines, 
  rgr_species_ranef_plot, rgr_distance_lines), 

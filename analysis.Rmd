...
(germ_hab_fig <- ggplot(germination_data, aes(f_habitat, pct_germinated)) +
  ...
  stat_summary(aes(color = common_name, group = common_name), 
               ...
               geom = c("pointrange"), size = .75, show.legend = T, 
               position = position_jitterdodge(jitter.width = .1)) +
  scale_color_brewer(palette = "BrBG") +
  facet_wrap(~age) +
  theme_bw() +
...
(surv_hab_fig <- ggplot(surv, aes(f_habitat, 100*survival)) +
  # geom_smooth(aes(linetype = common_name), method = "lm", se = F, show.legend = T, color = "grey50", size = .5) +
...
(diam_hab_fig <- ggplot(habitat_grouped_data, aes(f_habitat, avg_diameter)) +
  geom_line(aes(color = common_name, group = common_name)) +
...
(biomass_hab_fig <- ggplot(habitat_grouped_data, aes(f_habitat, avg_biomass)) +
  geom_line(aes(color = common_name, group = common_name)) +
  # geom_smooth(aes(linetype = status), method = "lm", se = F) +
...
(rgr_hab_fig <- ggplot(habitat_grouped_data, aes(f_habitat, avg_rgr_ht)) +
  geom_line(aes(color = common_name, group = common_name)) +
  # geom_smooth(aes(linetype = status), method = "lm", se = F) +
...
cowplot::plot_grid(
  ...
  theme(legend.position = "top",
        ...
        plot.margin = unit(c(.2,.2,0,.2), units = "cm")
        ) +
  ...
  theme(axis.text = element_text(size = 10),
        ...
        plot.margin = unit(c(0,.2,.2,.2), units = "cm")
        ),
  
  ncol = 1, rel_heights = c(1.3,1), labels = "auto"
)
ggsave("figures/germ-survival-raw.eps", width = 5, height = 5)
...
cowplot::plot_grid(
  ...
  diam_hab_fig +
    xlab("") +
    ggtitle("") +
    scale_y_continuous(limits = c(1.9,4.2)) +
    theme(axis.text = element_text(size = 10),
          strip.text = element_blank(),
          axis.title = element_text(size = 12),
          axis.text.x = element_blank(),
  ...
  rgr_hab_fig +
    # xlab("") +
    ggtitle("") +
    scale_y_continuous(breaks = c(2.5,3.0,3.5), 
                       labels = c("2.5","3.0","3.5"), 
                       limits = c(2.5,3.7)
                       ) +
    theme(axis.text = element_text(size = 10),
          strip.text = element_blank(),
          ...
          # axis.text.x = element_blank(),
          legend.position = "none",
          plot.margin = unit(c(-.4,.2,.2,.2), units = "cm")),
  
  ncol = 1, rel_heights = c(1.3,1,1), labels = "auto"
)
ggsave("figures/growth-raw.png", width = 4.5, height = 10)
...
cowplot::plot_grid(plotlist = list(
  ...
  nrow = 3, ncol = 2, rel_widths = c(1,1), labels = "auto")

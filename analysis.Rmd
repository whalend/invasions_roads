...
(germ_hab_fig <- ggplot(germination_habitat_data, aes(f_habitat, avg_germinated)) +
...
(surv_hab_fig <- ggplot(survival_habitat_data, aes(f_habitat, pct_survival)) +
  # geom_smooth(aes(linetype = common_name), method = "lm", se = F, show.legend = T, color = "grey50", size = .5) +
  geom_line(aes(color = common_name, group = common_name)) +
...
(biomass_hab_fig <- ggplot(habitat_grouped_data, aes(f_habitat, avg_biomass)) +
  geom_line(aes(color = common_name, group = common_name)) +
  # geom_smooth(aes(linetype = status), method = "lm", se = F) +
  geom_errorbar(aes(ymin = avg_biomass - se_biomass, ymax = avg_biomass + se_biomass, color = common_name), width = 0, alpha = .5, size = .75) +
  geom_point(aes(color = common_name), size = 2.5) +
  facet_wrap(~age) +
  scale_color_brewer(palette = "BrBG") +
  theme_bw() +

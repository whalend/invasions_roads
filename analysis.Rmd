...
survival_2006 <- surv %>% 
  filter(year==2006) %>% 
  group_by(site_code, age, distance, common_name, avg_basal_area, avg_pct_light, litter_kg_m2, pct_moisture) %>% 
...
We calculated the average biomass accumulated, height, and relative growth rate for height acros the four replicates of each species at each distance at each site. These average values were used as the response variables in regression models. 
...
biomass_mod1 <- nlme::gls(log(avg_biomass) ~ distance*avg_pct_light + avg_basal_area*age + distance*litter_kg_m2 + distance*pct_moisture, data = filter(site_grouped_data, is.na(avg_biomass)=="FALSE"), method = "REML")
...
Form <- formula(log(avg_biomass) ~ distance + avg_pct_light + age + distance*pct_moisture + (1|site_code) + (1|common_name))
# m4.full <- lmer(Form, data = site_grouped_data, REML = F)
# summary(m4.full)
...
# plot(biomass_mod4a@frame$distance, E2)
...
plot(biomass_model@frame$pct_moisture, E2)
...
biomass_model_scaled <- lmer(log(avg_biomass) ~ scaled_distance + scaled_pct_light + age + scaled_distance*scaled_pct_moisture + (1|site_code) + (1|common_name), data = site_grouped_data)
...
### Model coefficients
```{r biomass-ranef-plot}

# lattice::dotplot(fixef(biomass_model))
biomass_ranef <- ranef(biomass_model, condVar = T)
...
(biomass_species_ranef_plot <- ggplot(filter(biomass_ranef_df, grpvar=="common_name"), 
       aes(y = grp, x = condval)) +
  geom_point() +
  ...
  labs(x="", y="") +
  ggtitle("Species biomass intercepts"))
...
(biomass_site_ranef_plot <- ggplot(filter(biomass_ranef_df, grpvar=="site_code"), 
  ...
  labs(x="", y="") +
  ggtitle("Site biomass intercepts"))  


# lattice::qqmath(ranef(biomass_model, condVar = T))
```

```{r biomass-fixef-plot, cache=T}
# biomass_fixef <- fixef(biomass_model)
# str(biomass_fixef)
biomass_summary <- summary(biomass_model)
# str(biomass_summary)
var_names <- c("Intercept","Distance","Pct. light","Forest age: young","Pct. moisture","Distance:Pct. moisture")
...
biomass_confint <- biomass_confint[-c(1:3),]
...
(biomass_coef_plot <- ggplot(filter(biomass_fixed, var_names!="Intercept"), 
  ...
  labs(x="", y="") +
  ggtitle("Means ± 95% confidence intervals - biomass"))  
...
(biomass_scaled_coef_plot <- ggplot(filter(biomass_scaled_coef, var_names!="Intercept"), 
  ...
  labs(x="", y="") +
  ggtitle("(Scaled) means ± 95% confidence intervals - biomass")
  )
...
# library(sjPlot)
# plot_model(biomass_model, mdrt.values = "all")
...
Conditional and marginal R^2^ of the biomass model `r MuMIn::r.squaredGLMM(biomass_model)`
...
diam_model <- lmer(log(avg_diameter) ~ distance + avg_pct_light + age + distance*pct_moisture + (1|site_code) + (1|common_name), data = site_grouped_data)
summary(diam_model)
...
# maybe some pattern here with light availability, but difficult to make out
plot(diam_model@frame$pct_moisture, E2)


# model with scaled predictors ----
diam_model_scaled <- lmer(log(avg_diameter) ~ scaled_distance + scaled_pct_light + age + scaled_distance*scaled_pct_moisture +  (1|site_code) + (1|common_name), data = site_grouped_data)
summary(diam_model_scaled)

```

```{r diameter-ranef-plot}

diam_ranef <- ranef(diam_model, condVar = T)
...
(diam_species_ranef_plot <- ggplot(filter(diam_ranef_df, grpvar=="common_name"), 
       aes(y = grp, x = condval)) +
  geom_point() +
  geom_errorbarh(aes(xmin = condval - 2*condsd, xmax = condval + 2*condsd), height = 0) +
  geom_vline(xintercept = 0, color = "gray70") +
  ...
  labs(x="", y="") +
  ggtitle("Species diameter intercepts"))
...
(diam_site_ranef_plot <- ggplot(filter(diam_ranef_df, grpvar=="site_code"), 
  ...
  labs(x="", y="") +
  ggtitle("Site diameter intercepts"))

```

```{r diameter-fixef-plot, cache=T}
...
(diam_coef_plot <- ggplot(filter(diam_fixed, var_names!="Intercept"), 
  ...
  labs(x="", y="") +
  ggtitle("Means ± 95% confidence intervals - diameter"))
...
(diam_scaled_coef_plot <- ggplot(filter(diam_scaled_coef, var_names!="Intercept"), 
  ...
  labs(x="", y="") +
  ggtitle("(Scaled) means ± 95% confidence intervals - diameter")
  )
...
Conditional and marginal R^2^ estimated for the diameter model `r MuMIn::r.squaredGLMM(diam_model_scaled)`
...
form <- formula(avg_rgr_ht ~ avg_pct_light + age + distance * pct_moisture + (1 | site_code) + (1 | common_name))
...
Do we actually want to test/know what the slope/intercept interaction is between distance and species as a rondom component??

```{r final rgr model}

form <- formula(avg_rgr_ht ~ distance + avg_pct_light + age + distance * pct_moisture + (1 | site_code) + (1 | common_name))
rgr_model <- lmer(form, data = site_grouped_data, REML = T)
summary(rgr_model)
...
# maybe some pattern here with light availability, but difficult to make out
plot(rgr_model@frame$pct_moisture, E2)

rgr_model_scaled <- lmer(avg_rgr_ht ~ scaled_distance + scaled_pct_light + age + scaled_distance*scaled_pct_moisture + (1 | site_code) + (1 | common_name),
                         data = site_grouped_data)
...
rgr_ranef <- ranef(rgr_model, condVar = T)
...
(rgr_species_ranef_plot <- ggplot(filter(rgr_ranef_df, grpvar=="common_name"), 
       aes(y = grp, x = condval)) +
  geom_point() +
  geom_errorbarh(aes(xmin = condval - 2*condsd, xmax = condval + 2*condsd), height = 0) +
  geom_vline(xintercept = 0, color = "gray70") +
  ...
  labs(x="", y="") +
  ggtitle("Species RGR intercepts"))
...
(rgr_site_ranef_plot <- ggplot(filter(rgr_ranef_df, grpvar=="site_code"), 
  ...
  labs(x="", y="") +
  ggtitle("Site RGR intercepts"))

```

```{r rgr-fixef-plot, cache=T}
...
(rgr_coef_plot <- ggplot(filter(rgr_fixed, var_names!="Intercept"), 
  ...
  labs(x="", y="") +
  ggtitle("Means ± 95% confidence intervals - RGR"))
...
(rgr_scaled_coef_plot <- ggplot(filter(rgr_scaled_coef, var_names!="Intercept"), 
  ...
  labs(x="", y="") +
  ggtitle("(Scaled) means ± 95% confidence intervals - RGR")
  )
...
Conditional and marginal R^2^ estimated for the diameter model `r MuMIn::r.squaredGLMM(rgr_model_scaled)`
...
germ_model <- glmer(cbind(seeds_germinated,seeds_present) ~ distance + avg_pct_light + avg_basal_area * age + litter_kg_m2 + (1 | site_code) + (1 | common_name), data = germination_data, family = "binomial")
summary(germ_model)
...
germ_model_scaled <- glmer(cbind(seeds_germinated,seeds_present) ~ scaled_distance + scaled_pct_light + scaled_basal_area * age + scaled_litter_kg_m2 +  (1 | site_code) + (1 | common_name), data = germination_data, family = "binomial")
...
germ_ranef <- ranef(germ_model, condVar = T)
...
```{r germ-fixef-plot, cache=T}
germ_summary <- summary(germ_model)
# str(germ_summary)
var_names <- c("Intercept","Distance","Pct. light","Basal area","Forest age: young","Litter mass","Basal area:Forest age")
germ_fixed <- as.data.frame(germ_summary$coefficients)
germ_fixed$var_names <- var_names
system.time(germ_confint <- as.data.frame(confint(germ_model)))
...
write.csv(germ_scaled_coef, "model_output_data/germ_fixed_coef.csv")
...
surv_model <- glmer(cbind(alive,dead) ~ scaled_distance*scaled_pct_light + scaled_basal_area*age + scaled_distance*scaled_litter_kg_m2 + (1|site_code) + (1|common_name), data = survival_2006, family = "binomial")

summary(surv_model)
plot(surv_model)

e2 <- resid(surv_model, type = "deviance")

ggplot(as.data.frame(cbind(fitted(surv_model),e2)),
       aes(fitted(surv_model), e2)) +
  geom_point() +
  geom_smooth()

plot(survival_2006$scaled_distance, e2)
lines(lowess(e2~survival_2006$scaled_distance))
...
plot(survival_2006$scaled_litter_kg_m2, e2)
lines(lowess(e2~survival_2006$scaled_litter_kg_m2))
...
```{r surv-fixef-plot, cache=T}
surv_summary <- summary(surv_model)
# str(surv_summary)
var_names <- c("Intercept","Distance","Pct. light","Basal area","Forest age: young","Litter mass","Pct. light:Distance","Basal area:Forest age","Litter mass:Distance")
surv_fixed <- as.data.frame(surv_summary$coefficients)
surv_fixed$var_names <- var_names
surv_confint <- as.data.frame(confint(surv_model))
...
write.csv(surv_fixed, "model_output_data/survival_fixed_coef.csv")

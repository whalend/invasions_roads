...
This grouping of data would theoretically gives an average data point of the performance of each species at each distance at each site (n = `r 12*6*6`). What we actually have is 420 observations, so we have 12 that were missing, I believe lost due to mowing at the edge.

```{r where observations are missing}
xtabs(~site_code + distance + common_name, data = site_grouped_data)
```

We lost all seedlings at the 0 meter distance from the FR and HQ sites (n = 12), which means we have 10 instead of 12 observations for plants at 0 meters.
...
# In the model below I use that factored habitat instead of distance
...
# library(sjPlot)
# plot_model(biomass_model) + theme_classic

lattice::dotplot(ranef(biomass_mod6, condVar = T))
lattice::qqmath(ranef(biomass_mod6, condVar = T))

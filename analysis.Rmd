...
```{r herbiv-by-year}
ggplot(herbiv, aes(distance, herbivory)) +
...
bmod <- lmer(log(avg_biomass) ~ distance + avg_pct_light + age + (1|site_code) + (1|common_name), data = site_grouped_data)
...
summary(m1)
summary(m2)# indicates that % moisture has no effect after accounting for other variables
...
biomass_coefs <- as.data.frame(unlist(coef(biomass_model)))
...
Conditional and marginal R^2^ of the biomass model `r MuMIn::r.squaredGLMM(bmod)`
...
biomass_sjplot <- sjPlot::sjp.lmer(biomass_model, type = "ri.slope", legend.title = "")
...
biomass_distance <- biomass_sjplot$plot[[6]]
biomass_distance +
  # labs(y = "Biomass", x = "Distance") + 
  theme_classic()
...
```{r gridded-results-figure, fig.height=10, fig.width=7}

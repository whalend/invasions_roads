...
biomass_model_scaled <- lmer(log(avg_biomass) ~ scale(distance) + scale(avg_pct_light) + age + scale(distance)*scale(pct_moisture) + (1|site_code) + (1|common_name), data = site_grouped_data)
...
ggplot(filter(biomass_ranef_df, grpvar=="common_name"), 
       aes(y = grp, x = condval)) +
  geom_point() +
...
ggplot(filter(biomass_ranef_df, grpvar=="site_code"), 
       aes(y = grp, x = condval)) +
  geom_point() +
  geom_errorbarh(aes(xmin = condval - 2*condsd, xmax = condval + 2*condsd), height = 0) +
  geom_vline(xintercept = 0, color = "gray70") +
...
The native species generated less biomass than non-native species, with spicebush and dogwood performing particularly poorly. Privet and autumn olive were especially good at amassing biomass, with honeysuckle performing slightly less well. Only privet and spicebush had intercepts where the estimated error didn't cross zero.

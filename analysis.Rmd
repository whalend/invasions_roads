...
ggplot(herbiv, aes(distance, herbivory)) +
  ...
  # geom_point(aes(shape = common_name)) +
  geom_smooth(aes(linetype = status), method = "lm", se = T, show.legend = F, color = "grey50", alpha = .2) +
  scale_linetype_manual(values = c(2,1)) +
  scale_color_brewer(palette = "RdBu") +
  facet_grid(age~year) +
  theme_bw() +
...
I'm not actually sure how to fit a model to these data. It's ordinal in that the categories indicate the range of herbivory in terms of leaf area affected. 

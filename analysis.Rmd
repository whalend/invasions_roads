...
plot(biomass_model@frame$avg_h_incidence_06, E2)
...
site_grouped_data <- site_grouped_data %>% 
  mutate(scaled_distance = distance/max(distance),
         ...
         scaled_herbiv_incidence = (avg_h_incidence_06-mean(avg_h_incidence_06, na.rm = T))/(2*sd(avg_h_incidence_06, na.rm = T)))


biomass_model_scaled <- lmer(log(avg_biomass) ~ scaled_distance + scaled_pct_light + f_age + scaled_herbiv_incidence + (1|site_code) + (1|common_name), data = site_grouped_data)
...
sjPlot::sjp.lmer(bmod_herbiv6, type = "rs.ri", show.legend = T)
...
diam_model <- lmer(log(avg_diameter) ~ distance + avg_pct_light + age + avg_h_incidence_06 + (1|site_code) + (1|common_name), data = site_grouped_data)
summary(diam_model)
...
# maybe some pattern here with light availability, but difficult to make out
plot(diam_model@frame$avg_h_incidence_06, E2)


# model with scaled predictors ----
diam_model_scaled <- lmer(log(avg_diameter) ~ scaled_distance + scaled_pct_light + age + scaled_herbiv_incidence + (1|site_code) + (1|common_name), data = site_grouped_data)
...
diam_distance <- diam_sjplot$plot[[2]]
...
sjPlot::sjp.lmer(diam_model_h5, type = "rs.ri", show.legend = T)$plot +
  theme_bw
...
rgr_model <- lmer(avg_rgr_ht ~ distance + avg_pct_light + age + avg_h_incidence_06 + (1|site_code) + (1|common_name), data = site_grouped_data, REML = T)
summary(rgr_model)
...
# maybe some pattern here with light availability, but difficult to make out
plot(rgr_model@frame$avg_h_incidence_06, E2)

rgr_model_scaled <- lmer(avg_rgr_ht ~ scaled_distance + scaled_pct_light + age + scaled_herbiv_incidence + (1 | site_code) + (1 | common_name), data = site_grouped_data)
...
rgr_sjplot <- sjPlot::sjp.lmer(rgr_model, type = "ri.slope", vars = "distance", show.intercept = T, show.loess.ci = T)
rgr_distance <- rgr_sjplot$plot[[2]]
...
sjPlot::sjp.lmer(rmod_herbiv3, type = "rs.ri")# avg herbiv. incidence
# sjPlot::sjp.lmer(rmod_herbiv4, type = "rs.ri")# cumulative herbiv. incidence

```

## Multipanel figure of growth model results
```{r species-growth-diff-figure, fig.width=10}
biomass_species_ranef_plot$data$grp <- factor(biomass_species_ranef_plot$data$grp, c("Autumn olive","Honeysuckle", "Privet", "Dogwood", "Redbud", "Spicebush"))
biomass_species_ranef_plot <- biomass_species_ranef_plot + 
  ggtitle("") +
  geom_point(aes(color = grp), size = 3, show.legend = F) +
  scale_color_brewer(palette = 7)
  
diam_species_ranef_plot$data$grp <- factor(biomass_species_ranef_plot$data$grp, c("Autumn olive","Honeysuckle", "Privet", "Dogwood", "Redbud", "Spicebush"))
diam_species_ranef_plot <- diam_species_ranef_plot + 
  ggtitle("") +
  geom_point(aes(color = grp), size = 3, show.legend = F) +
  scale_color_brewer(palette = 7)

rgr_species_ranef_plot$data$grp <- factor(biomass_species_ranef_plot$data$grp, c("Autumn olive","Honeysuckle", "Privet", "Dogwood", "Redbud", "Spicebush"))
...
biomass_distance_lines <- biomass_distance_lines + 
  theme(legend.position = c(0.6, 0.95),
        legend.direction = "horizontal",
        legend.background = element_blank())
diam_distance_lines <- diam_distance_lines + theme(legend.position = "none")
rgr_distance_lines <- rgr_distance_lines + theme(legend.position = "none")

cowplot::plot_grid(plotlist = list(
  biomass_distance_lines, biomass_species_ranef_plot, 
  diam_distance_lines, diam_species_ranef_plot, 
  rgr_distance_lines, rgr_species_ranef_plot), 
  nrow = 3, ncol = 2, rel_widths = c(1,1))

ggsave(filename = "figures/distance-intercepts-opt2.png", width = 9, height = 9)

```

```{r growth-fixed-coefs-figure}

biomass_scaled_coef_plot <- biomass_scaled_coef_plot + 
  ggtitle("biomass")
diam_scaled_coef_plot <- diam_scaled_coef_plot + 
  ggtitle("diameter") +
  theme(axis.text.y = element_blank())
rgr_scaled_coef_plot <- rgr_scaled_coef_plot + 
  ggtitle("rgr") +
  theme(axis.text.y = element_blank(),
        panel.background = element_rect(margin(r = 2)))

cowplot::plot_grid(plotlist = list(
  biomass_scaled_coef_plot, diam_scaled_coef_plot, rgr_scaled_coef_plot
), nrow = 1, rel_widths = c(1.6,1,1))
ggsave("figures/growth-scaled-coefs-fig.png", width = 9, height = 4.5)
...
germ_sjplot <- sjPlot::sjp.glmer(germ_model, type = "ri.slope", facet.grid = F)
...
surv_sjplot <- sjPlot::sjp.glmer(surv_model, type = "ri.slope", facet.grid = F)
...
herb_ordinal <- ggplot(herbiv_noNA_data, aes(habitat_type, herbivory)) +
  # geom_boxplot(aes(group = distance)) +
  geom_point(aes(color = status), position = "jitter") +
  geom_smooth(aes(color = status), method = "lm", show.legend = F) +
  facet_grid(age~year) +
  # scale_color_brewer(palette = 7) +
  xlab("") +
  theme_bw() +
...
herb_incidence <- ggplot(herbiv_noNA_data, aes(habitat_type, herbivory_incidence)) +
  # geom_boxplot(aes(group = distance)) +
  geom_point(aes(color = status), position = "jitter") +
  geom_smooth(aes(color = status), method = "lm", show.legend = F) +
  facet_grid(age~year) +

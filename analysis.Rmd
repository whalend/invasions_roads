...
ggplot(survival_data, aes(distance, pct_survival)) +
  stat_summary(aes(shape = common_name), fun.data = "mean_se", geom = "point", size = 2.5) +
...
height_data <- seedling_data %>% 
  group_by(distance, status, age, common_name) %>% 
  summarise(avg_height = mean(final_ht06, na.rm = T),
            se_height = sd(final_ht06, na.rm = T)/sqrt(length(final_ht06)))

ggplot(height_data, aes(distance, avg_height)) +
...
diameter_data <- seedling_data %>% 
  group_by(distance, status, age, common_name) %>% 
  summarise(avg_diameter = mean(diameter06, na.rm = T),
            se_diameter = sd(diameter06, na.rm = T)/sqrt(length(diameter06)))

ggplot(diameter_data, aes(distance, avg_diameter)) +
  geom_point(aes(shape = common_name)) +
  geom_line(aes(linetype = common_name)) +
  facet_wrap(~age) +
  theme_bw() +
...
biomass_data <- seedling_data %>% 
  ...
  summarise(avg_biomass = mean(tot_biomass, na.rm = T),
            ...
            se_leaf_biomass = sd(leaf_biomass, na.rm = T)/sqrt(length(leaf_biomass)),
            avg_stem_biomass = mean(stem_biomass, na.rm = T),
            se_stem_biomass = sd(stem_biomass, na.rm = T)/sqrt(length(stem_biomass)),
            avg_stem_leaf_ratio = mean(stem_biomass/leaf_biomass, na.rm = T),
            se_stem_leaf_ratio = sd(stem_biomass/leaf_biomass, na.rm = T)/sqrt(length(stem_biomass)),)
...
rgr_data <- seedling_data %>%
  group_by(distance, status, age, common_name) %>% 
  summarise(avg_rgr_ht = mean(rgr_ht_days, na.rm = T),
            se_rgr_ht = sd(rgr_ht_days, na.rm = T)/sqrt(length(rgr_ht_days)))

ggplot(rgr_data, aes(distance, avg_rgr_ht)) +
  geom_point(aes(shape = common_name)) +
  geom_line(aes(linetype = common_name)) +
  facet_wrap(~age) +
  theme_bw() +
...
ggplot(herbiv_data, aes(distance, avg_herbivory)) +

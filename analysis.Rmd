...
fill_manual <- scale_fill_manual(values = c("grey90", "grey70", "grey50"))
...
litter_habitat_data <- litter_data %>% 
  ...
  summarise(avg_litter = mean(litter),
            se_litter = sd(litter)/sqrt(length(litter)))
...
habitat_grouped_data <- seedling_data %>% 
  mutate(age = ifelse(age=="young", "Y","M"),
         stem_leaf_ratio = stem_biomass/leaf_biomass) %>%
  left_join(., forest_habitat_data) %>% 
  left_join(., light_habitat_data) %>% 
  left_join(., litter_habitat_data) %>% 
  left_join(., soil_moisture_habitat_data) %>% 
...
forest_data$f_habitat <- factor(forest_data$habitat_type, levels = c("0","1","2"), labels = c("0-5 m","10-20 m","40-60 m"))
...
light_data$f_habitat <- factor(light_data$habitat_type, levels = c("0","1","2"), labels = c("0-5 m","10-20 m","40-60 m"))
...
litter_habitat_data$f_habitat <- factor(litter_habitat_data$habitat_type, levels = c("0","1","2"), labels = c("0-5 m","10-20 m","40-60 m"))
avg_litter_data <- litter_habitat_data %>% 
  group_by(age, f_habitat) %>% 
  summarise(avg_litter2 = mean(avg_litter),
            se_litter = sd(avg_litter)/length(avg_litter))

ggplot(avg_litter_data, aes(age, avg_litter2, fill = f_habitat)) +
  geom_col(position = "dodge") +
  geom_errorbar(aes(ymin = avg_litter2 - se_litter, ymax = avg_litter2 + se_litter), width = 0, position = position_dodge(width = .9)) +
  fill_manual +
  theme_classic() +
  theme(legend.title = element_blank()) +
  xlab("") +
...
ggplot(germination_distance_data, aes(distance, avg_germinated)) +
  geom_point(aes(shape = common_name), show.legend = T, size = 2) +
...
biomass_m1 <- lmer(log(avg_biomass) ~ (1|site_code) + (1|status/species) + (1|distance), data = distance_grouped_data)
plot(biomass_m1)
summary(biomass_m1)
...
biomass_m2 <- lmer(log(avg_biomass) ~ distance + (1|site_code) + (1|status/species), data = grouped_seedling_data)
plot(biomass_m2)
summary(biomass_m2)
...
biomass_m4 <- lmer(log(avg_biomass) ~ factor(habitat_type) + status + scale(avg_basal_area) + scale(avg_pct_light) + scale(pct_moisture) + scale(litter) + (1|site_code) + (1|species), data = d1)
plot(biomass_m4)
summary(biomass_m4)
...
d1 <- grouped_seedling_data %>% 
  mutate(habitat_type = ifelse(distance<10,0,ifelse(distance>20, 2, 1)))

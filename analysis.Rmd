...
biomass_model_scaled <- lmer(log(avg_biomass) ~ scaled_distance + scaled_pct_light + f_age + (1|site_code) + (1|common_name), data = site_grouped_data)
...
# I tried an herbivory predictor from each year using either averaged ordinal scores or average incidence (presence/absence), resulting in data pooled across species at each distance at each site. For the ordinal model, only the average herbivory from the 2004 observation was statistically significant. Using incidence, observations from 2005 and 2006 are statistically significant. 
...
biomass_summary <- summary(biomass_model)
# str(biomass_summary)
var_names <- c("Intercept","Distance","Pct. light","Forest age: young")
...
biomass_sjplot <- sjPlot::sjp.lmer(biomass_model, type = "ri.slope", vars = "distance")
...
diam_model <- lmer(log(avg_diameter) ~ distance + avg_pct_light + age + (1|site_code) + (1|common_name), data = site_grouped_data)
summary(diam_model)
...
diam_model_scaled <- lmer(log(avg_diameter) ~ scaled_distance + scaled_pct_light + age +  (1|site_code) + (1|common_name), data = site_grouped_data)
...
Seedling diameter increased with light availability and was greater in younger forests, but decreased with distance.

```{r sjplot-diameter}

diam_sjplot <- sjPlot::sjp.lmer(diam_model, type = "ri.slope", vars = "distance")
diam_sjplot <- diam_sjplot$plot[[2]]
diam_lines <- diam_sjplot +
  # geom_line() +
  # geom_point(data = site_grouped_data, aes(distance, log(avg_biomass))) +
  scale_color_brewer(palette = 7) +
  labs(y = "Diameter", x = "Distance", title = "") +
  theme_classic() +
  theme(legend.title = element_blank())
...
# there's strong correlation between distance and % moisture in the mixed model
rmod <- lmer(log(avg_diameter) ~ distance + avg_pct_light + age + (1|site_code) + (1|common_name), data = site_grouped_data)

anova(rmod, diam_mod4e)
...
form <- formula(avg_rgr_ht ~ distance + avg_pct_light + age + (1 | site_code) + (1 | common_name))
...
rgr_model_scaled <- lmer(avg_rgr_ht ~ scaled_distance + scaled_pct_light + age + (1 | site_code) + (1 | common_name),
...
rgr_sjplot <- sjPlot::sjp.lmer(rgr_model, type = "ri.slope", vars = "distance")
rgr_sjplot <- rgr_sjplot$plot[[2]]
rgr_lines <- rgr_sjplot +
  # geom_line() +
  # geom_point(data = site_grouped_data, aes(distance, log(avg_biomass))) +
  scale_color_brewer(palette = 7) +
  labs(y = "Relative growth rate", x = "Distance", title = "") +
  theme_classic() +
  theme(legend.title = element_blank())
...
```{r gridded-dotplots, fig.height=10, fig.width=7}

cowplot::plot_grid(plotlist = list(biomass_scaled_coef_plot, biomass_species_ranef_plot, diam_scaled_coef_plot, diam_species_ranef_plot, rgr_scaled_coef_plot, rgr_species_ranef_plot), nrow = 3,ncol = 2, axis = "lb")

```

```{r gridded-lineplots}

cowplot::plot_grid(plotlist = list(biomass_lines, diam_lines, rgr_lines),
                   ncol = 1)
...
```{r germ-fixef-plot-data, cache=T, eval=F}
...
germ_sjplot <- sjPlot::sjp.glmer(germ_model_scaled, type = "ri.slope", vars = "scaled_distance", facet.grid = F)
germ_sjplot <- germ_sjplot$plot[[2]]
germ_lines <- germ_sjplot +
  labs(y = "Germination probability", x = "Scaled distance") +
  ggtitle("") +
  theme_classic() +
  theme(legend.title = element_blank()) +
  scale_color_brewer(palette = 7)
...
surv_model <- glmer(cbind(alive,dead) ~ scaled_distance + age + scaled_litter_kg_m2 + (1|site_code) + (1|common_name), data = survival_2006, family = "binomial")
...
```{r surv-fixef-plot-data, cache=T, eval=F}
...
The `ordinal` package has the `clmm` and `clmm2`, with will probably suffice for my purposes. I just need to get a decent inference off the data. Two other packages implement Bayesian MCMC frameworsks:  [MCMCglmm](https://cran.r-project.org/package=MCMCglmm) and [brms](https://cran.r-project.org/package=brms).

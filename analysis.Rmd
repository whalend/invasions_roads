...
seedling_data <- seedling_data %>% 
  ...
  mutate(habitat_type = if_else(distance<10,0,ifelse(distance>20, 2, 1)),
         ...
         rgr_ht_year = (log(final_ht06)) - log((initial_ht))/3)
...
site_grouped_data <- seedling_data %>%
  ...
  summarise(
    ...
    avg_rgr_ht_yr = mean(rgr_ht_year, na.rm = T),
    se_rgr_ht_yr = sd(rgr_ht_year, na.rm = T)/sqrt(length(rgr_ht_year)),
...
ggplot(germination_data, aes(distance, pct_germinated)) +
  ...
  stat_summary(aes(color = common_name), fun.data = "mean_se", geom = "pointrange", size = .75) +
...
ggplot(germination_distance_data, aes(distance, avg_germinated)) +
  geom_point(aes(color = common_name), show.legend = T, size = 2.5) +
...
(germ_hab_fig <- ggplot(germination_data, aes(distance, pct_germinated)) +
  # geom_line(aes(color = common_name)) +
  # geom_point(aes(color = common_name), show.legend = F, size = 1) +
  # stat_summary(aes(group = common_name, color = common_name), 
  #              fun.y = mean, geom = "line", 
  #              # position = position_jitterdodge(jitter.width = 1),
  #              show.legend = F) +
  stat_summary(aes(color = common_name, group = common_name), 
               fun.data = "mean_se", fun.y = "mean", 
               position = position_jitter(width = .5),
               geom = c("pointrange"), size = .5, show.legend = T 
               ) +
  scale_color_brewer(palette = "BrBG") +
  scale_x_continuous(breaks = c(0,5,10,20,40,60)) +
  facet_wrap(~age) +
  theme_bw() +
  theme(strip.background = element_blank(),
        panel.grid = element_blank(),
...
survival_2006 <- surv %>% 
  filter(year==2006) %>% 
  group_by(site_code, age, distance, f_habitat, common_name, status, avg_basal_area, avg_pct_light, litter_kg_m2, pct_moisture) %>% 
...
ggplot(survival_distance_data, aes(distance, 100*survival)) +
  ...
  stat_summary(aes(color = common_name), fun.data = "mean_se", geom = "pointrange", size = .75, show.legend = T) +
  # scale_linetype_manual(values = c(2,1)) +
  scale_color_brewer(palette = "BrBG") +
  facet_grid(.~age) +
  theme_bw() +
...
(surv_hab_fig <- ggplot(surv, aes(distance, 100*survival)) +
  # geom_smooth(aes(linetype = common_name), method = "lm", se = F, show.legend = T, color = "grey50", size = .5) +
  # geom_line(aes(color = common_name, group = common_name)) +
  # stat_summary(aes(group = common_name, color = common_name), 
  #              fun.y = mean, geom = "line", 
  #              # position = position_jitterdodge(jitter.width = 1),
  #              show.legend = F) +
  stat_summary(aes(color = common_name, group = common_name), 
               ...
               position = position_jitter(width = .5),
               geom = c("pointrange"), size = .5, show.legend = T) +
  # geom_point(aes(color = common_name), size = 2.5, show.legend = T) +
  scale_color_brewer(palette = "BrBG") +
    scale_x_continuous(breaks = c(0,5,10,20,40,60)) +
  facet_grid(.~age) +
  theme_bw() +
...
(diam_hab_fig <- ggplot(seedling_data, aes(distance, diameter06)) +
  # stat_summary(aes(group = common_name, color = common_name), 
  #              fun.y = mean, geom = "line", 
  #              # position = position_jitterdodge(jitter.width = .1),
  #              show.legend = F) +
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_blank()) +
  # ylab("Diameter (mm)") +
  ylab(expression(paste("Diameter (mm)",{}^" "))) +
  xlab("Distance from edge")
)
...
ggplot(habitat_grouped_data, aes(f_habitat, avg_diameter)) +
  ...
  geom_errorbar(aes(ymin = avg_diameter - se_diameter, ymax = avg_diameter + se_diameter, color = common_name), width = 0, alpha = .5, size = .75) +
  geom_point(aes(color = common_name), size = 2.5) +
  facet_wrap(~age) +
  scale_color_brewer(palette = "BrBG") +
  theme_bw() +
  theme(strip.background = element_blank(),
...
(biomass_hab_fig <- ggplot(seedling_data, aes(distance, tot_biomass)) +
  # stat_summary(aes(group = common_name, color = common_name), 
               # fun.y = mean, geom = "line", 
               # position = position_jitterdodge(jitter.width = .1),
               # show.legend = F) +
  stat_summary(aes(color = common_name, group = common_name), 
               ...
               position = position_jitter(width = .5),
               show.legend = T) +
   # geom_smooth(aes(color = common_name, linetype = age, fill = common_name), method = "lm", alpha = .3, se = F) +
  scale_color_brewer(palette = "BrBG") +
  scale_x_continuous(breaks = c(0,5,10,20,40,60), labels = c("0","5","10","20","40","60")) +
  facet_wrap(~age) +
  theme_bw() +
  theme(strip.background = element_blank(),
        panel.grid = element_blank(),
  ...
  # ylab(expression(paste("RGR (% ", {year}^-1,")"))) +
  xlab("Distance from edge (m)")
)

# xtabs(~age+distance+common_name, data = site_grouped_data)
ggplot(site_grouped_data, aes(distance, avg_biomass)) +
  # geom_line(aes(linetype = common_name, group = common_name)) +
  # geom_smooth(aes(linetype = status), method = "lm", se = F) +
  # geom_point(aes(color = common_name), size = 1, alpha = .7) +
  stat_summary(aes(color = common_name, group = common_name), 
               fun.data = "mean_se", fun.y = "mean", 
               ...
               show.legend = T) +
  scale_x_continuous(breaks = c(0,5,10,20,40,60), labels = c("0","5","10","20","40","60")) +
  # geom_errorbar(aes(ymin = avg_biomass - se_biomass, ymax = avg_biomass + se_biomass, color = common_name), width = 0) +
  # geom_smooth(aes(color = common_name, linetype = age, fill = common_name), method = "lm", alpha = .3, se = T) +
  facet_grid(.~age) +
  scale_color_brewer(palette = "BrBG") +
...
ggplot(habitat_grouped_data, aes(f_habitat, avg_biomass)) +
  geom_line(aes(color = common_name, group = common_name)) +
  # geom_smooth(aes(linetype = status), method = "lm", se = F) +
  geom_errorbar(aes(ymin = avg_biomass - se_biomass, ymax = avg_biomass + se_biomass, color = common_name), width = 0, size = .75) +
  geom_point(aes(color = common_name), size = 2.5) +
  facet_wrap(~age) +
  scale_color_brewer(palette = "BrBG") +
  theme_bw() +
...
(rgr_hab_fig <- ggplot(seedling_data, aes(distance, rgr_ht_year)) +
  # stat_summary(aes(group = common_name, color = common_name), 
               # fun.y = mean, geom = "line", 
               # position = position_jitterdodge(jitter.width = .1),
               # show.legend = F) +
...
ggplot(habitat_grouped_data, aes(f_habitat, avg_rgr_ht)) +
  geom_line(aes(color = common_name, group = common_name)) +
  # geom_smooth(aes(linetype = status), method = "lm", se = F) +
  geom_errorbar(aes(ymin = avg_rgr_ht - se_rgr_ht, ymax = avg_rgr_ht + se_rgr_ht, color = common_name), width = 0, alpha = .5, size = .75) +
  geom_point(aes(color = common_name), size = 2.5) +
  facet_wrap(~age) +
  scale_color_brewer(palette = "BrBG") +
  theme_bw() +
...
ggplot(herbiv_habitat_data, aes(f_habitat, avg_herbivory)) +
  geom_errorbar(aes(ymin = avg_herbivory - se_herbivory, ymax = avg_herbivory + se_herbivory, color = common_name), width = 0, alpha = .5, size = .75) +
  geom_line(aes(color = common_name, group = common_name), show.legend = F) +
  geom_point(aes(color = common_name), size = 2.5) +
  scale_color_brewer(palette = "BrBG") +
  facet_wrap(~age) +
  theme_bw() +
...
ggplot(herbiv, aes(f_habitat, herbivory)) +
  stat_summary(aes(color = common_name, shape = age), fun.data = "mean_se", geom = "pointrange", size = .6, show.legend = T, position = position_jitter(width = .2)) +
  # geom_point(aes(shape = common_name)) +
...
Herbivory was categorized as leaf-loss, where 0 = no herbivory, 1 = ≤25%, 2 = 26-50%, 3 = 51-75%, 4 = 76-99%, and 5 = 100% leaf removal by herbivores.
...
plot_grid(
  ...
  scale_color_brewer(palette = "BrBG", breaks=c("Autumn olive","Dogwood","Honeysuckle","Redbud","Privet","Spicebush"))) +
    draw_label("a", x = .09, y = .752, fontface = "bold") +
    draw_label("b", x = .55, y = .752, fontface = "bold"),
  ...
  theme(axis.text = element_text(size = 10),
    ...
    draw_label("c", x = .09, y = .981, fontface = "bold") +
    draw_label("d", x = .55, y = .981, fontface = "bold"),
  
  ncol = 1, rel_heights = c(1.3,1)
)
ggsave("figures/germ-survival-raw.eps", width = 7, height = 7)

cowplot::plot_grid(
  
  ggdraw(biomass_hab_fig +
    xlab("") +
    ggtitle("") +
    # lims(y = c(0,3.5)) +
    # scale_y_continuous(breaks = c(1,2,3), labels = c("1.0","2.0","3.0")) +
    theme(axis.text = element_text(size = 10),
          strip.text = element_text(size = 12),
          axis.title = element_text(size = 12),
          axis.text.x = element_blank(),
          legend.position = "top",
          legend.text = element_text(size = 10),
          plot.margin = unit(c(.1,.2,-.2,.2), units = "cm")
          ) +
    scale_color_brewer(palette = "BrBG", breaks=c("Autumn olive","Dogwood","Honeysuckle","Redbud","Privet","Spicebush"))) +
    draw_label("a", x = .095, y = .64, fontface = "bold") +
    draw_label("b", x = .56, y = .64, fontface = "bold"),
  ...
  ggdraw(diam_hab_fig +
    ...
    draw_label("c", x = .095, y = .91, fontface = "bold") +
    draw_label("d", x = .56, y = .91, fontface = "bold"),
  ...
  ggdraw(rgr_hab_fig +
    ...
    theme(axis.text = element_text(size = 9.5),
          ...
          plot.margin = unit(c(-.4,.2,.2,.13), units = "cm")
          )) +
    draw_label("e", x = .095, y = .95, fontface = "bold") +
    draw_label("f", x = .56, y = .95, fontface = "bold"),
  
  ncol = 1, rel_heights = c(1.3,1,1), axis = "l", align = "v"
)
ggsave("figures/growth-raw.eps", width = 6, height = 10)
...
library(GGally)
ggpairs(select(site_grouped_data, avg_biomass, distance, tree_richness, avg_basal_area, avg_pct_light, litter_kg_m2, pct_moisture, age, status), aes(color = age, alpha = .2), binwidth = 15)
...
m1 <- lmer(log(avg_biomass) ~ distance + avg_pct_light + f_age + avg_h_incidence + (1|site_code) + (avg_h_incidence|common_name), data = site_grouped_data) 
m2 <- lmer(log(avg_biomass) ~ distance + avg_pct_light + f_age + status + avg_h_incidence + (1|site_code) + (avg_h_incidence|common_name), data = site_grouped_data)
...
arm::display(biomass_model)
biomass_armstandardize_model <- arm::standardize(biomass_model)
arm::display(biomass_armstandardize_model)
...
biomass_scaled_coef$var_names <- factor(biomass_scaled_coef$var_names, levels = c("Herbivory","Pct. light","Forest age: young","Distance","Intercept"))
...
# lattice::dotplot(coef(biomass_armstandardize_model))

# coef(biomass_armstandardize_model)
...
biomass_sjplot <- sjPlot::sjp.lmer(biomass_model_scaled, type = "ri.slope", prnt.plot = F)
...
# biomass_sjplot$plot[[6]] +
#   scale_color_brewer(palette = "BrBG") +
#   theme_classic()

# sjPlot::sjp.lmer(biomass_armstandardize_model, type = "ri.slope")

# sjPlot::sjp.lmer(biomass_model, type = "fe.slope")
biomass_distance <- biomass_sjplot$plot[[5]]
biomass_distance$data$x <- biomass_distance$data$x*60
...
(biomass_distance_lines <- biomass_distance +
  geom_line(size = 1) +
  # geom_point(data = site_grouped_data, aes(distance, avg_biomass)) +
  scale_color_brewer(palette = "BrBG") +
  labs(y = "Predicted biomass (g)", x = "Distance (m)", title = "") +
  theme_classic() +
  theme(legend.title = element_blank())
)
...
# biomass_herbiv_rsri$data$x <- biomass_herbiv_rsri$data$x*(2*sd(site_grouped_data$avg_h_incidence, na.rm=T)) + mean(site_grouped_data$avg_h_incidence, na.rm=T)
biomass_herbiv_rsri$data$y = exp(biomass_herbiv_rsri$data$y)
## Effectively no difference in the plots whether using the unscaled data model or back-transforming the scaled explanatory variables.
...
diam_model_scaled <- lmer(log(avg_diameter) ~ scaled_distance + scaled_pct_light + f_age + scaled_herbiv_incidence + (1|site_code) + (scaled_herbiv_incidence|common_name), data = site_grouped_data)
...
**Coefficient estimates (±95% confidence intervals) on the diameter growth response**
...
```{r marginal-diameter-distance-plot}

diam_sjplot_rislope <- sjPlot::sjp.lmer(diam_model_scaled, type = "ri.slope", prnt.plot = F)
diam_distance <- diam_sjplot_rislope$plot[[5]]

diam_distance$data$x <- diam_distance$data$x*60
...
(diam_distance_lines <- diam_distance +
  ...
  # geom_point(data = site_grouped_data, aes(distance, log(avg_biomass))) +
  scale_color_brewer(palette = "BrBG") +
  labs(y = "Predicted diameter (mm)", x = "Distance", title = "") +
  theme_classic() +
  theme(legend.title = element_blank())
  )
...
rgr_model <- lmer(avg_rgr_ht_yr ~ distance + avg_pct_light + age + avg_h_incidence + (1|site_code) + (1|common_name), data = site_grouped_data, REML = T)
summary(rgr_model)
...
rgr_model_scaled <- lmer(avg_rgr_ht_yr ~ scaled_distance + scaled_pct_light + age + scaled_herbiv_incidence + (1|site_code) + (1|common_name), data = site_grouped_data)
# summary(rgr_model_scaled)

```

### Plots of random effects from RGR model
...
### Plot of fixed effects from RGR model
...
**Scaled coefficient estimates (relative effects) ±95% confidence intervals for effects of distance, forest age, available light, and herbivory on relative growth rate**
...
rgr_sjplot <- sjPlot::sjp.lmer(rgr_model_scaled, type = "ri.slope", prnt.plot = F)
rgr_distance <- rgr_sjplot$plot[[5]]
rgr_distance$data$x <- rgr_distance$data$x*60
...
Removed the random slope, random intercept with species and herbivory for this variable because there was no relationship.
```{r rgr-herbiv-by-species, eval=F}


rgr_rsri_sjplot <- sjPlot::sjp.lmer(rgr_model, type = "rs.ri", show.legend = T, prnt.plot = F)# avg herbiv. incidence
...
```{r fig2-species-growth-diff, fig.width=6, fig.height=7.5}
...
cowplot::plot_grid(plotlist = list(
  ...
  nrow = 3, ncol = 2, rel_widths = c(1,1), labels = "auto", align = "r", axis = "r")

# ggsave(filename = "figures/distance-intercepts-fig2.eps", width = 6, height = 7.5)

```

**Fig. 2.:** Differences between group intercepts and overall model intercepts (a, c, e) and the marginal effect of distance on growth metrics for each species (b, d, f).

Data table to check intercept values on the figure.

```{r fig2 data table check}

ranef_df <- rbind(filter(biomass_ranef_df, grpvar=="common_name",term=="(Intercept)") %>% 
                    mutate(response = "biomass"),
      filter(diam_ranef_df, grpvar=="common_name",term=="(Intercept)") %>% 
        mutate(response = "diameter"),
      filter(rgr_ranef_df, grpvar=="common_name",term=="(Intercept)") %>% 
        mutate(response = "rgr")) %>% 
  select(grp:response)

kable(ranef_df, digits = 3)
...
**Fig. 3.:** Random-slope-intercept plots for the marginal effect of herbivory on the biomass (a) and diameter (b) of each species. The different slopes illustrate species specific responses to herbivory, where some increase growth while others maintain or have less growth.


## Growth performance results
Seedling growth (biomass, diameter, and relative growth rate of height) all responded similarly to edge effects, light availability, and forest successional age. Overall, herbivory was statistically insignificant but the effect was species dependent. Growth increased with light availability, was greater in young forests, and decreased with distance from the forest edge (Fig. 1). Although the nonnative species generally outperformed the native species there were species specific differences among growth metrics after accounting for distance from edge, forest age, light availability, and herbivory. The nonnative autumn olive, honeysuckle, and especially privet produced more above-ground biomass relative to the overall average, while the native dogwood and especially spicebush generated relatively small amounts of biomass (Fig. 2a). The biomass of the native redbud was near the overall average across all species. Privet also had the greatest diameter follwed by honeysuckle, while dogwood and especially spicebush had relatively small diameters (Fig. 2c). The diameters of the native redbud and nonnative autumn olive were near the overall average. Autumn olive had the highest relative growth rate in height followed by privet, and spicebush had the lowest. Relative growth rates of native redbud and dogwood and nonnative honeysuckle were near the overall average across all species (Fig. 2e).
...
**Random intercepts for species (above) and site (below) for the germination probability**
...
#         ) +
#   labs(x="", y="") +
#   ggtitle("Means ± 95% confidence intervals - germ"))
...
germ_scaled_coef$var_names <- factor(germ_scaled_coef$var_names, levels = c("Litter mass","Forest age: young","Distance","Intercept"))
...
(germ_scaled_coef_plot <- ggplot(filter(germ_scaled_coef, var_names!="Intercept"), 
  ...
  labs(x="", y="") +
  ggtitle("(Scaled) means ± 95% confidence intervals - germ")
  )

```

**Plot marginal effects of distance and litter on germination for each species**
...
surv_model_scaled <- glmer(cbind(alive,dead) ~ scaled_distance + scaled_pct_light + age + scaled_litter_kg_m2 + (1|site_code) + (1|common_name), data = survival_2006, family = "binomial")
...
**Plots of random effects intercepts of species (above) and site (below) for the survival response**
...
**Plot main effects of distance, forest age, and litter mass on survival**
Because no main effects were statistically significant during model fitting we used the same explanatory variables as germination to compare and contrast these two components of establishment.

```{r surv-fixef-plot}

surv_fixed <- read_csv("model_output_data/survival_fixed_coef.csv")
surv_fixed$var_names <- factor(surv_fixed$var_names, levels = c("Litter mass","Forest age: young","Distance","Intercept"))
...
(surv_coef_plot <- ggplot(filter(surv_fixed, var_names!="Intercept"),
  ...
  labs(x="", y="") +
  ggtitle("Survival scaled means ± 95% CI"))
...
#         ) +
#   labs(x="", y="") +
#   ggtitle("(Scaled) means ± 95% confidence intervals - surv")
#   )
...
**Plot marginal effect of distance on survival**
...
Germination and survival differed in response to main effects, were species dependent, and for some species showed opposing patterns. Germination probability was significantly greater with increasing distance from forest edge, trended to be lower in young forest, and greater with higher litter mass. In contrast, survival was not significantly influenced by explanatory variables (Fig. 4 a & b). Species differences accounted for much of the variation in germination and survival, though most species did not show equal success in both responses. Autumn olive had the highest germination but the lowest seedling survival. Dogwood showed a similar pattern of relatively greater germination but lower survival. Privet had the opposite pattern of greater survival, though not significantly different from the average, and lower germination. Redbud and spicebush both had low germination and survival near the overall average. Honeysuckle stands out as having the greatest survival and above average germination, trailing only autumn olive.
...
```{r fig4-germ-survival-fixef}

germ_coef_plot <- germ_scaled_coef_plot +
  ggtitle("Germination")

surv_coef_plot <- surv_coef_plot +
  ggtitle("Survival") +
  theme(axis.text.y = element_blank())

cowplot::plot_grid(germ_coef_plot, surv_coef_plot,
                   rel_widths = c(1,.65), labels = "auto")

ggsave("figures/germ-survival-fixef-fig4.eps", width = 6, height = 3)

```


```{r fig5-germ-survival-ranef}
...
# ggsave("figures/germ-surv-ranef-fig5.eps", height = 6, width = 6)
...
```{r ordinal-herbivory-model, eval=F}

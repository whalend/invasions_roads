...
Other studies have looked at incidence, i.e., presence/absence of herbivory at each observation.

```{r find-herbivory-model, eval=F}
...
herbiv_noNA_data <- herbiv_noNA_data %>% 
  mutate(herbivory_incidence = as.factor(ifelse(herbivory == 0,0,1)))

ggplot(filter(herbiv_noNA_data), aes(distance, herbivory_incidence)) +
  # geom_boxplot(aes(group = distance)) +
  geom_point(aes(color = status), position = "jitter") +
  geom_smooth(aes(color = status), method = "loess") +
  facet_grid(age~year) +
